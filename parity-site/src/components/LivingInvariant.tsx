...
export default function LivingInvariant({ mini = false }: { mini?: boolean }) {
  ...
  return (
    <section className={`w-full bg-[var(--surface)] border-[var(--hairline)] ${mini ? 'py-2 border-b' : 'py-12 border-t'}`} ref={ref}>
      <div className={`max-w-7xl mx-auto px-6 ${mini ? 'scale-90 origin-left' : ''}`}>
        <div className={`flex flex-col md:flex-row items-center justify-between font-mono mono-num text-[var(--ink)] ${mini ? 'text-lg md:text-xl' : 'text-2xl md:text-3xl'}`}>
          <div>{format(val1)}</div>
          <div ref={equalsRef} className="my-4 md:my-0 font-display font-bold px-4">=</div>
